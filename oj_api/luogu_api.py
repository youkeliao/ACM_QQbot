...
class LUOGU(Contest):
    ...
    async def get_contest_info(self):
        ...
        for i in range(contest_len):
            ...
            res += await self.format_luogu_contest(next_contest, durationSeconds)
        return res.rstrip('\n')
    ...
    async def get_recent_info(self):
        ...
        return "洛谷比赛还有15分钟就开始啦，没有报名的尽快报名~\n" + recent

    async def format_luogu_contest(self, next_contest, durationSeconds):
        res = "比赛名称：{}\n" \
              "开始时间：{}\n" \
              "持续时间：{}\n" \
              "比赛地址：{}\n".format(
